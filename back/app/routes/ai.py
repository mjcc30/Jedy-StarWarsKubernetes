...
@router.get("/image")
async def get_entity_image(name: str, type: str, force_refresh: bool = False, session: Session = Depends(get_session)):
    if not is_ai_enabled():
    ...
    if not force_refresh:
        db_image = session.exec(select(EntityImage).where(EntityImage.entity_name == name)).first()
        
        if db_image:
            # If stored as URL (pollinations fallback)
            if db_image.image_url.startswith("http"):
                return Response(content=requests.get(db_image.image_url).content, media_type="image/png")
            # If stored as base64 (Gemini)
            return Response(content=base64.b64decode(db_image.image_url), media_type="image/png")
    ...
    if image_data:
        ...
        # Update or Create
        existing_img = session.exec(select(EntityImage).where(EntityImage.entity_name == name)).first()
        if existing_img:
            existing_img.image_url = b64_str
            existing_img.description = "Regenerated by Gemini 2.5"
            session.add(existing_img)
        else:
            new_image = EntityImage(entity_name=name, image_url=b64_str, description="Generated by Gemini 2.5")
    ...
    if existing_img:
        ...
        new_image = EntityImage(entity_name=name, image_url=fallback_url, description="Pollinations Fallback")
        session.add(new_image)
